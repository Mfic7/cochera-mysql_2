const ReservationForm = (() => {
  ...
  function cacheEls() {
    [
      ...
      "cancelacion-codigo",
      "cancelacion-motivo",
      "cancelacion-numero-operacion",
      "cancelacion-comprobante",
      "btn-solicitar-cancelacion",
      "cancelacion-banner-error",
      "stepper",
      "banner-error",
      "info-direccion",
      "info-horario",
  ...
  async function init() {
    ...
    if (brandName) {
      brandName.textContent =
        state.config.nombre_negocio || brandName.textContent;
    }
    ...
    document.title =
      (state.config.nombre_negocio || "Mi Cochera") + " — Reserva tu espacio";

    state.metodosPago = await Api.metodosPago();

    await reloadGrid();

    ["fecha", "hora", "horas"].forEach((id) =>
    ...
    el["btn-abrir-cancelacion"].addEventListener(
      "click",
      mostrarPanelCancelacion,
    );
    el["btn-solicitar-cancelacion"].addEventListener(
      "click",
  ...
  async function reloadGrid() {
    try {
      ...
      el["stat-tarifa"].textContent =
        "S/ " + Number(data.tarifa_hora).toFixed(2);
      ...
      el["parking-grid"].innerHTML =
        '<p class="loading">No se pudo cargar el mapa de espacios.</p>';
    }
  }
  ...
  function actualizarResumen() {
    ...
    el["r-espacio"].textContent = state.selectedEspacio
      ? state.selectedEspacio.codigo
      : "—";
  ...
  function setFormDisabled(disabled) {
    ["fecha", "hora", "horas", "cliente-nombre", "cliente-celular"].forEach(
      (id) => {
  ...
  function validarCelular(celular) {
    ...
    if (!/^9\d{8}$/.test(limpio))
      return "Ingresa un celular válido de 9 dígitos (ej. 987654321).";
    return null;
  }
  ...
  async function crearReserva() {
    hideError();
    if (!state.selectedEspacio)
      return showError("Selecciona un espacio disponible.");
  ...
  function onHoldExpired() {
    showError(
      "El tiempo para completar el pago expiró. El espacio fue liberado.",
    );
  ...
  function etiquetaMetodo(tipo) {
    return (
      { yape: "Yape", plin: "Plin", transferencia: "Transferencia" }[tipo] ||
      tipo
    );
  }
  ...
  function seleccionarMetodo(tipo) {
    ...
    const monto = state.reserva
      ? Number(state.reserva.monto_adelanto).toFixed(2)
      : "0.00";
    ...
    if (metodo.qr_image_path) {
      ...
      qrHtml =
        '<div class="qr-placeholder" title="El administrador aún no subió el código QR"></div>';
    }
  ...
  async function enviarComprobante() {
    ...
    try {
      ...
      el["panel-pago"]
        ...
        .forEach((n) => n.setAttribute("disabled", "disabled"));
      ...
      el["btn-enviar-comprobante"].textContent =
        "🔒 Enviar comprobante y reservar";
    }
  }

  // Reactiva el formulario de pago cuando el admin rechaza el comprobante,
  // para que el cliente pueda subir uno nuevo sin recargar la página.
  function reactivarFormularioPago() {
    el["panel-pago"]
      .querySelectorAll("input, .tab")
      .forEach((n) => n.removeAttribute("disabled"));
    el["numero-operacion"].value = "";
    el["comprobante"].value = "";
    el["btn-enviar-comprobante"].disabled = false;
    el["btn-enviar-comprobante"].textContent =
      "🔒 Enviar comprobante y reservar";
    el["hold-timer"].hidden = false;
    if (state.reserva.hold_expira_en) {
      HoldTimer.start(
        state.reserva.hold_expira_en,
        el["hold-timer"],
        onHoldExpired,
      );
    }
    ...
    if (estado === "cancelada" || estado === "vencida") {
      showError(
        estado === "cancelada"
          ...
          : "La reserva venció por falta de pago.",
      );
    }

    // Detecta que el admin RECHAZÓ el comprobante: pasamos de 'en_validacion'
    // de vuelta a 'pendiente_pago'. Mostramos el motivo y reactivamos el pago.
    if (
      state.lastEstado === "en_validacion" &&
      estado === "pendiente_pago" &&
      state.reserva.ultimo_rechazo
    ) {
      const motivo =
        state.reserva.ultimo_rechazo.motivo || "No cumple con lo requerido.";
      showToast(
        "❌ Tu comprobante fue rechazado. Motivo: " +
          motivo +
          ". Por favor, envía un nuevo comprobante.",
        25000,
      );
      reactivarFormularioPago();
    }

    // Si la reserva acaba de pasar a 'adelanto_pagado', notificar al usuario
    if (
      state.lastEstado !== "adelanto_pagado" &&
      estado === "adelanto_pagado"
    ) {
      showToast(
        "Tu reserva ha sido confirmada. Tienes 20 minutos antes de la hora reservada para cancelar. Pasado ese tiempo, no habrá devolución del dinero.",
        20000,
      );
    }
  ...
  function startPolling() {
    ...
    state.pollHandle = setInterval(async () => {
      try {
        const reserva = await Api.obtenerReserva(
          state.reserva.id,
          state.reserva.token,
        );
        state.reserva = { ...state.reserva, ...reserva };
        actualizarStepper(reserva.estado);
        if (
          ["pago_completo", "cancelada", "vencida"].includes(reserva.estado)
        ) {
          stopPolling();
        }
      } catch (e) {
  ...
  function mostrarPanelCancelacion() {
    ...
    el["panel-cancelacion"].scrollIntoView({
      behavior: "smooth",
      block: "nearest",
    });
  }
  ...
  function actualizarCancelacionPanel() {
    ...
    const estadosConfirmados = ["adelanto_pagado", "pago_completo"];
    const pagoConfirmado = estadosConfirmados.includes(state.reserva.estado);
  ...
  async function solicitarCancelacion() {
    ...
    if (!numeroOperacion) {
      return showCancelacionError(
        "Ingresa el número de operación o código del pago.",
      );
    }
    if (!comprobante) {
    ...
    try {
      ...
      el["btn-solicitar-cancelacion"].textContent =
